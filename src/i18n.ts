...
export function webviewStrings(lang: Lang): Record<string, string> {
  const ko: Record<string, string> = {
    ...
    codexModelNote: "이 PC 로컬 Codex 로그 기준 · 다른 PC/브라우저/원격 환경 사용분은 빠질 수 있음 · 7일 미사용 모델 자동 제외 · 비용은 표시하지 않음 · Codex는 출력과 추론 토큰을 합산해 표시합니다.",
  ...
  const en: Record<string, string> = {
    ...
    codexModelNote: "Based on this PC's local Codex logs · other PC/browser/remote usage may be missing · models unused in 7d auto-excluded · cost is not shown · Codex output and reasoning tokens are shown together.",
    localScanFailed: "Local log scan failed · showing last successful values",
    colOutReason: "Output+Reasoning",
    colEvents: "Events",
