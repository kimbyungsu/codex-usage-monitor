...
- **캐시 효율**: Claude는 적중률 도넛 + "캐시 덕에 절약한 추정 비용"(캐시 쓰기 할증을 차감한 순절감), Codex는 적중률 도넛(Codex의 `cached_input`은 `input`의 부분집합 의미론. 절약액 $는 새 모델/별칭이 자주 바뀌어 단가 자동 매칭을 안정적으로 보장할 수 없어 산출하지 않음)
